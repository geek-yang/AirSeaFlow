...
def amet(t, q, v, z, ps, lev, lat, lon):
    ...
    for i in np.arange(len(dp_level)):
        ...
        for j in np.arange(len(lat)):
            cpT_interim =  constant['cp'] * t[i,j,:] * v[i,j,:] * dp_level[i] / constant['g'] * dx[j]
            gz_interim = z[i,j,:] * v[i,j,:] * dp_level[i] / constant['g'] * dx[j]
            Lvq_interim = constant['Lv'] * q[i,j,:] * v[i,j,:] * dp_level[i] / constant['g'] * dx[j]
            # correction based on terrain
            cpT_interim[lev[i]>ps[j,:]] = 0
            gz_interim[lev[i]>ps[j,:]] = 0
            Lvq_interim[lev[i]>ps[j,:]] = 0
            # get the values
            cpT[i,j,:] = cpT_interim
            gz[i,j,:] = gz_interim
            Lvq[i,j,:] = Lvq_interim
...
if __name__=="__main__":
    ...
    for i in period:
    	for j in index_month:
        	...
        	t, q, v, z, ps = retriver(var_key, level*100)
        	cpT, gz, Lvq, E = amet(t, q, v, z, ps, level*100, lat, lon)
