...
Function        : Quantify stationary and transient eddy from atmospheric kinetic energy (ERA-Interim)(HPC-cloud customised)(temporal mean against monthly mean)
Author          : Yang Liu
Date            : 2019.09.28
...
Description     : The code aims to calculate the time and space dependent components
                  ...
                  !! Temporal mean deviates from monthly mean
                  !! Spatial mean deviates from monthly mean
...
def initialization(benchmark):
    ...
    # create variables
    v_temporal_sum = np.zeros((12,Dim_latitude,Dim_longitude),dtype=float)
    T_temporal_sum = np.zeros((12,Dim_latitude,Dim_longitude),dtype=float)
    q_temporal_sum = np.zeros((12,Dim_latitude,Dim_longitude),dtype=float)
    z_temporal_sum = np.zeros((12,Dim_latitude,Dim_longitude),dtype=float)
...
def pick_var(var_key):
    ...
    return v_daily, T_daily, q_daily, z_daily
...
def initialization_eddy(v_temporal_mean, T_temporal_mean,
    ...
    var_v_steady_mean_monthly_zonal_mean = np.mean(v_temporal_mean,2)
    var_T_steady_mean_monthly_zonal_mean = np.mean(T_temporal_mean,2)
    var_q_steady_mean_monthly_zonal_mean = np.mean(q_temporal_mean,2)
    var_z_steady_mean_monthly_zonal_mean = np.mean(z_temporal_mean,2)
...
def compute_eddy(var_v_temporal_mean, var_T_temporal_mean,
                 var_q_temporal_mean, var_z_temporal_mean,
    ...
    var_v_temporal_mean_select = np.repeat(var_v_temporal_mean[np.newaxis,:,:], seq, 0)
    var_T_temporal_mean_select = np.repeat(var_T_temporal_mean[np.newaxis,:,:], seq, 0)
    var_q_temporal_mean_select = np.repeat(var_q_temporal_mean[np.newaxis,:,:], seq, 0)
    var_z_temporal_mean_select = np.repeat(var_z_temporal_mean[np.newaxis,:,:], seq, 0)
    # calculate transient eddies
    ################# transient eddy ###################
    print ("Calculate transient eddies!")
    var_v_prime = var_v - var_v_temporal_mean_select # deviates from monthly mean
...
def create_netcdf_point_eddy(var_cpT_overall,var_cpT_transient,var_cpT_transient_mean,
    ...
    data_wrap = Dataset(os.path.join(output_path,'model_daily_075_E_eddies_{0}hPa_point_subtract_monthly.nc'.format(name_list[lev_slice])),'w',format = 'NETCDF4')
...
if __name__=="__main__":
    ...
    for i in period:
        for j in index_month:
            ...
            v_temporal_sum[j-1,:,:] = v_temporal_sum[j-1,:,:] + np.mean(var_v,0)
            T_temporal_sum[j-1,:,:] = T_temporal_sum[j-1,:,:] + np.mean(var_T,0)
            q_temporal_sum[j-1,:,:] = q_temporal_sum[j-1,:,:] + np.mean(var_q,0)
            z_temporal_sum[j-1,:,:] = z_temporal_sum[j-1,:,:] + np.mean(var_z,0)
    # calculate the temporal mean
    v_temporal_mean = v_temporal_sum / Dim_period # shape[12,lat,lon]
    ...
    for i in period:
        for j in index_month:
            ...
            = compute_eddy(v_temporal_mean[j-1,:,:], T_temporal_mean[j-1,:,:],
                           q_temporal_mean[j-1,:,:], z_temporal_mean[j-1,:,:],
                           var_v, var_T, var_q, var_z)
